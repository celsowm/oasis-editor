...
| Run properties | `w14:glow` | `w:rPr` | effect attrs | Glow halo around glyphs. | Partial: model+import+export+HTML (`text-shadow` 0-offset)+canvas (`shadowBlur`); no PDF approximation. `mc:AlternateContent` empty fallback. | P4 | Partial |
| Run properties | `w14:reflection` | `w:rPr` | effect attrs | Mirrored copy of glyphs below baseline. | Partial: model+import+export+canvas (mirrored pass, averaged alpha); no CSS/PDF rendering. `mc:AlternateContent` empty fallback. | P4 | Partial |
