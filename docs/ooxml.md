...
| Modern (w14/w15/w16) typography — remaining effects | `w14:textShadow`, `w14:glow`/`w14:reflection`, `w14:scene3d`/`w14:props3d`, `w15:collapsed`, `w16du:dateUtc`. OpenType features (`w14:ligatures` etc.) are **Partial** (Phase 1). `w14:textFill` is **Partial** (solid done; gradient PDF deferred). `w14:textOutline` is **Supported**. See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). |
