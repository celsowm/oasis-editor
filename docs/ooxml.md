...
| Main document | `w:sectPr` | `w:body`, `w:pPr` | `w:rsidR`, `w:rsidSect` | Section properties. | Can appear at end of body or inside a paragraph's `pPr` to end a section. Both locations are now detected on import; the body-direct sectPr ends the last section and a `w:pPr`-scoped sectPr ends an intermediate section. | P0 | Supported |
...
| Section/page | `w:type` | `w:sectPr` | `w:val` | Section break type. | Values include nextPage, continuous, evenPage, oddPage, nextColumn. Parsed from both body-direct and paragraph-scoped (`w:pPr`) sectPr with the OOXML off-by-one applied so `section[i].breakType` describes how section *i* begins; `continuous` drives real layout (no page break between sections); `nextPage`/`evenPage`/`oddPage`/`nextColumn` are preserved for round-trip. | P1 | Partial |
...
| Section/page | `w:pgNumType` | `w:sectPr` | `w:start`, `w:fmt`, `w:chapStyle`, `w:chapSep` | Page numbering format. | Needed for generated page numbers and fields. Parsed and re-emitted for round-trip (`start`, `fmt`, `chapStyle`, `chapSep`); `start` is not yet wired into PAGE-field numbering. | P2 | Partial |
...
| Section/page | `w:bidi` | `w:sectPr` | `w:val` | Bidi section layout. | Affects page flow and margins in RTL sections. Parsed and re-emitted for round-trip; not yet applied to layout/margins. | P2 | Partial |
...
| Section/page | `w:vAlign` | `w:sectPr` | `w:val` | Vertical justification of page contents. | Values top/center/both/bottom; affects page vertical placement. Parsed and re-emitted for round-trip (`top` is the default and is normalized away); not yet applied to page vertical placement. | P2 | Partial |
...
| Run properties | `w:kern` | `w:rPr` | `w:val` | Kerning threshold (min font size). Round-trips; canvas hints `ctx.fontKerning` when the run size meets the threshold; PDF applies real GPOS pair kerning above the threshold. | Text measurement fidelity. | P2 | Supported |
...
| Area | Element / tag / part | Parent / context | Key attributes | Meaning | Importer notes | Priority | Status |
|---|---|---|---|---|---|---|---|
| Content controls | `w:sdt` | Block/run/table row/cell contexts | — | Structured document tag wrapper. | **Block-level** `w:sdt` (body + text-box content) round-trips: content is unwrapped into the normal block flow and the wrapper is preserved on each block (incl. nested controls) and re-emitted. Run-level (inline), table-row/cell, and dropped-content scenarios are not yet handled. No content-control UI/widgets. | P1 | Partial |
| Content controls | `w:sdtPr` | `w:sdt` | — | Content control properties. | Preserved verbatim for block-level controls (so `w:tag`/`w:alias`/`w:id`/`w:dataBinding`/`w15:collapsed`/etc. survive round-trip), but not interpreted. | P1 | Partial |
| Content controls | `w:sdtContent` | `w:sdt` | — | Content control content. | Contains normal block/run/row/cell content depending control type. | P1 | Partial |
...
| Run properties | `w14:ligatures` | `w:rPr` | `w14:val` | OpenType ligature control. | Round-tripped (import → model → export), emitted as CSS `font-variant-ligatures` for HTML, and applied in PDF via the GSUB shaper (`liga`/`calt`/`hlig`). Canvas keeps only a coarse `textRendering` hint (no Canvas 2D feature API). See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P3 | Partial |
| Run properties | `w14:numForm` | `w:rPr` | `w14:val` | Number form typography. | Round-tripped, emitted as CSS `font-variant-numeric` for HTML, and applied in PDF via GSUB (`lnum`/`onum`). Canvas not applied (no Canvas 2D API). See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P4 | Partial |
| Run properties | `w14:numSpacing` | `w:rPr` | `w14:val` | Number spacing typography. | Round-tripped, emitted as CSS `font-variant-numeric` for HTML, and applied in PDF via GSUB (`pnum`/`tnum`). Canvas not applied (no Canvas 2D API). See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P4 | Partial |
| Run properties | `w14:stylisticSets` / `w14:stylisticSet` | `w:rPr` | `w14:id`, `w14:val` | OpenType stylistic set selection. | Round-tripped, emitted as CSS `font-feature-settings` for HTML, and applied in PDF via GSUB (`ss01`–`ss20`). Canvas not applied (no Canvas 2D API). See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P4 | Partial |
| Run properties | `w14:cntxtAlts` | `w:rPr` | `w14:val` | Contextual alternates. | Round-tripped, emitted as CSS `font-feature-settings:"calt"` for HTML, and applied in PDF via GSUB (`calt`). Canvas not applied (no Canvas 2D API). See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P4 | Partial |
| Run properties | `w14:textFill` | `w:rPr` | DrawingML fill children | Modern text fill. | Solid fill: fully supported (import → model → canvas → PDF → HTML). Gradient: import/export/HTML/canvas plus PDF (axial Type 2 shading painted through a glyph clip) done; per-stop alpha and radial gradients deferred. Export wraps in `mc:AlternateContent` with `w:color` fallback. See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P3 | Partial |
| Run properties | `w14:textOutline` | `w:rPr` | line/fill attrs | Modern text outline. | Fully supported: import → model → canvas (fill+stroke) → PDF (Tr mode 2) → HTML (`-webkit-text-stroke`). Export wraps in `mc:AlternateContent` with `w:outline` fallback. See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). | P3 | Supported |
| Run properties | `w14:shadow` (modern shadow) | `w:rPr` | effect attrs | Modern text shadow. | Partial: model+import+export+HTML (`text-shadow`)+canvas (`shadowBlur/OffsetX/Y`)+PDF offset copy; `mc:AlternateContent` with `w:shadow` fallback. | P4 | Partial |
| Run properties | `w14:glow` | `w:rPr` | effect attrs | Glow halo around glyphs. | Partial: model+import+export+HTML (`text-shadow` 0-offset)+canvas (`shadowBlur`)+PDF (8-offset copy approximation, alpha baked into color). `mc:AlternateContent` empty fallback. | P4 | Partial |
| Run properties | `w14:reflection` | `w:rPr` | effect attrs | Mirrored copy of glyphs below baseline. | Partial: model+import+export+HTML+canvas (vertically mirrored pass, avg alpha, optional blur)+PDF (downward-shifted copy, alpha baked into color; no flip). No CSS standard for text-reflect. `mc:AlternateContent` empty fallback. | P4 | Partial |
...
| Sections | Page size (`w:pgSz`), page margins (`w:pgMar`), header/footer references (default/even/first), the section `docGrid`, and paragraph-scoped `w:sectPr` (intermediate section breaks nested in `w:pPr` are now detected so multi-section documents don't collapse). `w:type` `continuous` is rendered — continuous sections flow without a page break. |
...
| Theme color (`a:clrScheme`) | `a:clrScheme` slots (`dk1`, `lt1`, `accent1`…) are parsed from `word/theme/theme1.xml`. Run text color (`w:color`) and shading (`w:shd`) resolve `w:themeColor`/`w:themeTint`/`w:themeShade` to concrete hex on import. Border colors that reference the theme and `w:clrSchemeMapping` overrides in `settings.xml` are not applied. | Border theme colors and per-document scheme remapping are not resolved. |
...
| Section properties | `w:type` (section break type) is parsed from both body-direct and paragraph-scoped (`w:pPr`) `w:sectPr` with the OOXML off-by-one applied; `continuous` drives real layout (sections flow without a page break), while `nextPage`/`evenPage`/`oddPage`/`nextColumn` are preserved for round-trip. `w:pgNumType` (`start`/`fmt`/`chapStyle`/`chapSep`), `w:vAlign` (center/both/bottom; `top` normalized away), and section `w:bidi` are parsed and re-emitted for round-trip. Paragraph-scoped `w:sectPr` (intermediate section breaks) is now detected so multi-section Word documents don't collapse into one section. | `pgNumType.start` is not yet wired into PAGE-field numbering; `vAlign`/`bidi`/`evenPage`/`oddPage`/`nextColumn` are not yet applied to layout — they are round-trip preservation only. |
...
| AltChunk | `w:altChunk`. |
| Bidi/RTL/Complex script | `w:rtl`, `w:cs`, `w:bdo`/`w:dir`, `w:lang`, `w:bidiVisual`, `w:noColumnBalance`. (Section-level `w:bidi` is now parsed/round-tripped — see Sections.) |
...
| Columns / page borders / text direction | `w:pageBorders`, `w:textDirection` (cell/section), `w:lnNumType`, `w:paperSrc`, `w:formProt`, `w:noEndnote`, `w:printerSettings`. (`w:type`, `w:pgNumType`, `w:vAlign` now round-trip — see Sections.) |
...
| Modern (w14/w15/w16) typography — remaining effects | `w14:scene3d`/`w14:props3d` are **round-trip only** (preserved verbatim, not rendered). `w16du:dateUtc` is **round-trip only** on comments. `w15:collapsed` is **Not supported** — blocked on `w:sdt` infrastructure, which is itself not yet imported (see Content controls rows). OpenType features (`w14:ligatures` etc.) are **Partial** (Phase 1). `w14:textFill` is **Partial** (solid done; gradient PDF deferred). `w14:textOutline` is **Supported**. `w14:shadow`/`w14:glow`/`w14:reflection` are **Partial** (Phase 3). See [w14-modern-typography-roadmap.md](w14-modern-typography-roadmap.md). |
