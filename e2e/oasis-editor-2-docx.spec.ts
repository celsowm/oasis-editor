...
test.describe("Oasis Editor 2 DOCX", () => {
  ...
  test("imports a DOCX with paragraph and table content", async ({ page }) => {
    ...
    await page.locator('[data-testid="editor-2-import-docx-input"]').setInputFiles({
      name: "import.docx",
      mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      buffer: file,
    });

    await expect(page.locator('[data-testid="editor-2-block"]').first()).toContainText("Imported paragraph");
    await expect(page.locator('[data-testid="editor-2-table-cell"]').first()).toContainText("A1");
    await expect(page.locator('[data-testid="editor-2-table-cell"]').nth(1)).toContainText("B1");
  });
  ...
  test("exports the imported DOCX back to a downloadable file", async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toBe("oasis-editor-2.docx");
