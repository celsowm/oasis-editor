...
export function extractMarkupCompatibilityMetadata(
  element: XmlElement,
): PreservedMcMetadata {
  ...
  const ignorable =
    element.getAttributeNS(MARKUP_COMPATIBILITY_NS, "Ignorable") ||
    element.getAttribute("mc:Ignorable");
  if (ignorable) metadata.ignorable = ignorable;

  const processContent =
    element.getAttributeNS(MARKUP_COMPATIBILITY_NS, "ProcessContent") ||
    element.getAttribute("mc:ProcessContent");
  if (processContent) metadata.processContent = processContent;

  const preserveElements =
    element.getAttributeNS(MARKUP_COMPATIBILITY_NS, "PreserveElements") ||
    element.getAttribute("mc:PreserveElements");
  if (preserveElements) metadata.preserveElements = preserveElements;

  const preserveAttributes =
    element.getAttributeNS(MARKUP_COMPATIBILITY_NS, "PreserveAttributes") ||
    element.getAttribute("mc:PreserveAttributes");
  ...
  for (let i = 0; i < element.childNodes.length; i++) {
    ...
    if (child && child.nodeType === child.ELEMENT_NODE) {
      ...
      if (
        elem.namespaceURI === MARKUP_COMPATIBILITY_NS &&
        elem.localName === "AlternateContent"
      ) {
        metadata.alternateContentBlocks.push(
...
export function serializeMcAttributes(metadata: PreservedMcMetadata): string {
  ...
  if (metadata.processContent)
    attrs.push(`mc:ProcessContent="${metadata.processContent}"`);
  if (metadata.preserveElements)
    attrs.push(`mc:PreserveElements="${metadata.preserveElements}"`);
  if (metadata.preserveAttributes)
    attrs.push(`mc:PreserveAttributes="${metadata.preserveAttributes}"`);
  return attrs.join(" ");
}
