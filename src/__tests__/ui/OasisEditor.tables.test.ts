...
describe("OasisEditor", () => {
  ...
  it("moves between imported table cells with tab and shift plus tab", async () => {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelectorAll('[data-testid="editor-table-cell"]').length === 2) {
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    expect(root.querySelectorAll('[data-testid="editor-table-cell"]').length).toBe(2);
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelectorAll('[data-testid="editor-table-cell"]').length === 2) {
        const cellTexts = Array.from(root.querySelectorAll('[data-testid="editor-table-cell"]')).map(
    ...
    let cells = Array.from(root.querySelectorAll('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
    ...
    cells = Array.from(root.querySelectorAll('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
  ...
  it("keeps tab navigation in tables even when the active cell paragraph is a list item", async () => {
    ...
    const cells = Array.from(root.querySelectorAll('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
  ...
  it("splits a table cell with enter and keeps the table structure", async () => {
    ...
    importInput.dispatchEvent(new Event("change", { bubbles: true }));
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelectorAll('[data-testid="editor-table-cell"]').length === 1) {
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    ...
    const cell = root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement;
    expect(cell.querySelectorAll('[data-testid="editor-block"]').length).toBe(2);
    expect(root.querySelectorAll('[data-testid="editor-table-cell"]').length).toBe(1);
  ...
  it("backspaces inside a table cell without collapsing the table", async () => {
    ...
    const cell = root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement;
    expect(cell.querySelectorAll('[data-testid="editor-block"]').length).toBe(1);
  ...
  it("moves vertically between table cells with arrow up and arrow down", async () => {
    ...
    const firstParagraph = root.querySelector('[data-testid="editor-table-cell"] [data-paragraph-id]') as HTMLElement;
  ...
  it("merges and splits table cells horizontally through the toolbar", async () => {
    ...
    let cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]'));
    ...
    cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]'));
  ...
  it("merges table cells contextually through the toolbar", async () => {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelector('[data-testid="editor-table-selection-label"]')?.textContent?.includes("Table selection: 2 cells")) {
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    expect(root.querySelector('[data-testid="editor-table-selection-label"]')?.textContent).toContain("Table selection: 2 cells");
  ...
  it("splits table rows contextually through the toolbar", async () => {
    ...
    const cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]'));
    expect(cells.length).toBe(2);
  ...
  it("inserts and deletes table rows through the advanced table actions", async () => {
    ...
    importInput.dispatchEvent(new Event("change", { bubbles: true }));
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelectorAll('[data-testid="editor-table-cell"]').length === 4) {
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    ...
    let rows = root.querySelectorAll('[data-testid="editor-table-row"]');
    expect(rows.length).toBe(3);

    let cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
    ...
    rows = root.querySelectorAll('[data-testid="editor-table-row"]');
    expect(rows.length).toBe(2);
    cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
    expect(cells).toEqual(["A1", "A2", "B1", "B2"]);
  ...
  it("inserts and deletes table columns through the advanced table actions", async () => {
    ...
    let rows = Array.from(root.querySelectorAll('[data-testid="editor-table-row"]'));
    expect(rows.length).toBe(2);
    let cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
    ...
    rows = Array.from(root.querySelectorAll('[data-testid="editor-table-row"]'));
    expect(rows.length).toBe(2);
    cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
  ...
  it("inserts and deletes table columns in vertically merged tables", async () => {
    ...
    importInput.dispatchEvent(new Event("change", { bubbles: true }));
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelectorAll('[data-testid="editor-table-cell"]').length === 3) {
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    ...
    let rowTexts = rows.map((row) =>
      Array.from(row.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
        (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
      ),
    );
  ...
  it("inserts and deletes table rows and columns in mixed-span tables", async () => {
    ...
    expect(root.querySelectorAll('[data-testid="editor-table-row"]').length).toBe(3);
    expect((root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement).getAttribute("rowspan")).toBe("3");

    deleteRowButton.click();
    await Promise.resolve();

    expect(root.querySelectorAll('[data-testid="editor-table-row"]').length).toBe(2);
    expect((root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement).getAttribute("rowspan")).toBe("2");
  ...
  it("inserts and deletes table rows in vertically merged tables", async () => {
    ...
    let rows = root.querySelectorAll('[data-testid="editor-table-row"]');
    expect(rows.length).toBe(3);
    let firstCell = root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement;
    ...
    rows = root.querySelectorAll('[data-testid="editor-table-row"]');
    expect(rows.length).toBe(2);
    firstCell = root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement;
  ...
  it("moves vertically past continued cells created by a row merge", async () => {
    ...
    const cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
  ...
  it("highlights the visible range of a mixed-span table selection", async () => {
    ...
    const paragraphs = root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"] [data-paragraph-id]');
    ...
    expect(root.querySelectorAll('[data-testid="editor-selection-box"]').length).toBe(3);
  ...
  it("backspaces within a multi-block table cell without flattening the cell", async () => {
    ...
    const cellParagraphs = root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"] [data-paragraph-id]');
    ...
    const firstCell = root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement;
    expect(firstCell.querySelectorAll('[data-testid="editor-block"]').length).toBe(1);
    const cells = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]')).map(
      (cell) => cell.textContent?.replace(/\u00A0/g, "") ?? "",
    );
  ...
  it("splits a multi-block table cell with enter at a block boundary and keeps the table structure", async () => {
    ...
    const firstCell = root.querySelector('[data-testid="editor-table-cell"]') as HTMLElement;
    expect(firstCell.querySelectorAll('[data-testid="editor-block"]').length).toBe(3);
  ...
  it("applies bold to the full visual table-cell range including the last cell", async () => {
    ...
    const runs = root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"] [data-testid="editor-run"]');
  ...
  it("inserts a 3x3 table into the document on Insert Table click", async () => {
    ...
    const cells = root.querySelectorAll('[data-testid="editor-table-cell"]');
