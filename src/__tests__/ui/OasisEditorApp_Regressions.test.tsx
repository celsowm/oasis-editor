...
describe("useEditorPersistence Regressions", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("should unwrap and JSON-serialize the document to avoid DataCloneError", async () => {
    ...
    await createRoot(async (dispose) => {
      // Initialize hook
      useEditorPersistence(state() as any, vi.fn());
  ...
  it("should NOT throw when clicking an image (ReferenceError regression)", () => {
    // This is a structural test to ensure imports and scoping in OasisEditorApp.tsx are valid.
    // Since OasisEditorApp is a complex component, we rely on the fact that 
