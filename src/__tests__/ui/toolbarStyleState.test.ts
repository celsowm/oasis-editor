...
import type { EditorDocument, EditorState, EditorNamedStyle } from "../../core/model.js";
...
function makeState(overrides: Partial<EditorState> & { document: EditorDocument }): EditorState {
  return {
    selection: {
...
describe("getToolbarStyleState — golden (current static behavior)", () => {
  it("[GOLDEN] resolves named text style for a run with styleId", () => {
    resetEditorIds();
    const paragraph = createEditorParagraphFromRuns([{ text: "hello", styles: { styleId: "Heading1" } }]);
  ...
  it("[GOLDEN] resolves paragraph named styles across selection with mixed styleIds", () => {
    resetEditorIds();
    const paragraphs = [
      createEditorParagraphFromRuns([{ text: "a" }]),
      createEditorParagraphFromRuns([{ text: "b" }]),
    ];
