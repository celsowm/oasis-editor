...
describe("EditorSurface", () => {
  it("renders separate DOM runs for a multi-run paragraph", () => {
    ...
    const runNodes = container.querySelectorAll('[data-testid="editor-run"]');
    const charNodes = container.querySelectorAll('[data-testid="editor-char"]');
  ...
  it("applies inline run styles from the semantic AST", () => {
    ...
    const runNode = container.querySelector('[data-testid="editor-run"]') as HTMLSpanElement;
  ...
  it("renders inline image runs as img elements with preserved dimensions", () => {
    ...
    const image = container.querySelector(".oasis-editor-image") as HTMLImageElement;
    const chars = container.querySelectorAll('[data-testid="editor-char"]');
  ...
  it("renders the same paragraph across multiple paginated blocks", () => {
    ...
    const pages = container.querySelectorAll('[data-testid="editor-page"]');
  ...
  it("renders paginated page containers for the projected document layout", () => {
    ...
    expect(container.querySelectorAll('[data-testid="editor-page"]').length).toBeGreaterThan(1);
  ...
  it("renders semantic tables as a real grid with paragraph content in cells", () => {
    ...
    const state: EditorState = {
      document: createEditorDocument([intro, table]),
      selection: {
        anchor: {
    ...
    expect(container.querySelector('[data-testid="editor-table"]')).not.toBeNull();
    expect(container.querySelectorAll('[data-testid="editor-table-row"]').length).toBe(1);
    expect(container.querySelectorAll('[data-testid="editor-table-cell"]').length).toBe(2);
  ...
  it("renders paginated table segments with repeated header rows on continued pages", () => {
    ...
    const table = createEditorTable([
      createEditorTableRow(
        [createEditorTableCell([createEditorParagraphFromRuns([{ text: "Header".repeat(18) }])])],
        { isHeader: true },
      ),
      ...Array.from({ length: 20 }, (_, index) =>
        createEditorTableRow([
          createEditorTableCell([
            createEditorParagraphFromRuns([{ text: `Body${index}`.repeat(28) }]),
          ]),
        ]),
      ),
    ]);
    const state: EditorState = {
      document: createEditorDocument([table]),
      selection: {
        anchor: {
    ...
    const pages = container.querySelectorAll('[data-testid="editor-page"]');
    const tables = container.querySelectorAll('[data-testid="editor-table"]');
  ...
  it("renders page and surface dimensions from document page settings", () => {
    ...
    expect(page.classList.contains("oasis-editor-paper-landscape")).toBe(true);
