...
import { createOasisEditorContainer } from "../../app/bootstrap/createOasisEditorContainer.js";
...
describe("OasisEditor", () => {
  ...
  it("renders the isolated editor shell and input overlay", () => {
    ...
    expect(root.querySelector('[data-testid="editor-editor"]')).not.toBeNull();
    expect(root.querySelector('[data-testid="editor-surface"]')).not.toBeNull();
    expect(input).not.toBeNull();
    expect(input.style.pointerEvents).toBe("none");
    expect(root.querySelector('[data-testid="editor-toolbar-bold"]')).not.toBeNull();
    expect(root.querySelector('[data-testid="editor-toolbar-font-family"]')).not.toBeNull();
    expect(root.querySelector('[data-testid="editor-toolbar-align-center"]')).not.toBeNull();
    expect(root.querySelector('[data-testid="editor-toolbar-list-bullet"]')).not.toBeNull();
    expect(root.querySelector('[data-testid="editor-toolbar-undo"]')).not.toBeNull();
    expect(root.querySelector('[data-testid="editor-toolbar-redo"]')).not.toBeNull();
    expect(root.querySelectorAll('[data-testid="editor-block"]').length).toBe(1);
  ...
  it("renders the public container without demo chrome and emits state changes", async () => {
    ...
    const instance = createOasisEditorContainer(root, {
      viewportHeight: 480,
      class: "test-public-container",
      onStateChange,
    });
    const shell = root.querySelector('[data-testid="editor-editor-shell"]') as HTMLDivElement;
    const editor = root.querySelector('[data-testid="editor-editor"]') as HTMLDivElement;
    const input = root.querySelector('[data-testid="editor-input"]') as HTMLTextAreaElement;
    ...
    expect(root.querySelector(".oasis-editor-header")).toBeNull();
    expect(root.querySelector(".oasis-editor-toolbar")).toBeNull();

    input.value = "ab";
    input.dispatchEvent(new InputEvent("input", { bubbles: true, data: "ab", inputType: "insertText" }));
    await Promise.resolve();

    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain("ab");
  ...
  it("repositions the caret from a click and inserts at the clicked offset", async () => {
    ...
    const block = root.querySelector('[data-testid="editor-block"]') as HTMLParagraphElement;
  ...
  it("expands selection with shift plus arrow left", async () => {
    ...
    const selectedChars = root.querySelectorAll(".oasis-editor-char-selected");
    expect(selectedChars.length).toBe(1);
    expect(selectedChars[0]?.textContent).toBe("b");
    expect(root.querySelectorAll('[data-testid="editor-selection-box"]').length).toBeGreaterThan(0);
  ...
  it("supports undo and redo from the keyboard", async () => {
    ...
    await Promise.resolve();
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toBe("\u00A0");
  ...
  it("supports undo and redo from the toolbar", async () => {
    ...
    const instance = createOasisEditor(root);
    const input = root.querySelector('[data-testid="editor-input"]') as HTMLTextAreaElement;
    const undoButton = root.querySelector('[data-testid="editor-toolbar-undo"]') as HTMLButtonElement;
    const redoButton = root.querySelector('[data-testid="editor-toolbar-redo"]') as HTMLButtonElement;
  ...
  it("moves to the start and end of the current paragraph with home and end", async () => {
    ...
    await Promise.resolve();
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain("hello!");
  ...
  it("moves by word with ctrl plus arrow left and right", async () => {
    ...
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain(
      "hello brave ^world",
    );
  ...
  it("moves a selected inline image to the next paragraph with alt plus arrow down", async () => {
    ...
    try {
      ...
      expect(paragraphs[1]?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("moves a selected inline image to the previous paragraph with alt plus arrow up", async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const imageAfterMove = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement | null;
  ...
  it("moves a selected inline image inside a table cell to the next table cell with alt plus arrow down", async () => {
    ...
    try {
      ...
      const image = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement;
      expect(image).not.toBeNull();
      expect(image.closest('[data-testid="editor-table-cell"]')).toBe(firstCellBefore);
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        ...
        if (movedImage && movedImage.closest('[data-testid="editor-table-cell"]') === secondCellBefore) {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }
      ...
      expect(cellsAfter[0]?.querySelector('[data-testid="editor-image"]')).toBeNull();
      expect(cellsAfter[1]?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("commits IME composition once and ignores the duplicate input event", async () => {
    ...
    input.value = "Ã¡";
    input.dispatchEvent(new InputEvent("input", { bubbles: true, data: "Ã¡", inputType: "insertCompositionText" }));
    await Promise.resolve();
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toBe("\u00A0");

    input.dispatchEvent(new CompositionEvent("compositionend", { bubbles: true, data: "Ã¡" }));
    await Promise.resolve();
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain("Ã¡");

    input.value = "Ã¡";
    input.dispatchEvent(new InputEvent("input", { bubbles: true, data: "Ã¡", inputType: "insertText" }));
    await Promise.resolve();
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain("Ã¡");
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).not.toContain("Ã¡Ã¡");
  ...
  it("highlights an empty block when a multi-block selection spans through it", async () => {
    ...
    const selectedEmpty = root.querySelector(".oasis-editor-empty-char-selected");
  ...
  it("cuts and pastes selected text", async () => {
    ...
    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain("hell");
  ...
  it("pastes rich html content with inline styles and links", async () => {
    ...
    const paragraphs = root.querySelectorAll('[data-testid="editor-block"]');
