...
import { buildDocx, createTinyPngFile, setupOasisEditorDom } from "./oasisEditorTestHarness.js";

describe("OasisEditor", () => {
  beforeEach(() => {
    setupOasisEditorDom();
  });
  ...
  it("pastes an inline image from the clipboard", async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        if (root.querySelector('[data-testid="editor-image"]')) {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }

      const image = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement | null;
  ...
  it("pastes plain text when ctrl plus shift plus v is used", async () => {
    ...
    try {
      ...
      const runs = Array.from(root.querySelectorAll('[data-testid="editor-run"]')) as HTMLElement[];
  ...
  it("edits alt text for the selected inline image through the toolbar", async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const altBtn = root.querySelector('[data-testid="editor-toolbar-image-alt"]');
      ...
      (root.querySelector('[data-testid="editor-toolbar-image-alt"]') as HTMLButtonElement | null)?.click();

      for (let attempt = 0; attempt < 20; attempt += 1) {
        if (document.querySelector('.oasis-editor-dialog-input')) {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }

      const altInput = document.querySelector('.oasis-editor-dialog-input') as HTMLInputElement | null;
      expect(altInput).not.toBeNull();
      
      if (altInput) {
        altInput.value = "Chart alt";
        altInput.dispatchEvent(new Event("input", { bubbles: true }));
        const saveBtn = document.querySelector('.oasis-editor-dialog-button-primary') as HTMLButtonElement | null;
        saveBtn?.click();
      }

      for (let attempt = 0; attempt < 20; attempt += 1) {
        if (root.querySelector('[data-testid="editor-image"]')?.getAttribute("alt") === "Chart alt") {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }

      const updatedImage = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement | null;
  ...
  it("edits alt text for the selected inline image with ctrl plus alt plus a", async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        if (root.querySelector('[data-testid="editor-image"]')?.getAttribute("alt") === "Shortcut alt") {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }

      expect(root.querySelector('[data-testid="editor-image"]')?.getAttribute("alt")).toBe("Shortcut alt");
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("supports undo and redo after pasting an inline image", async () => {
    ...
    try {
      ...
      expect(root.querySelector('[data-testid="editor-image"]')).not.toBeNull();

      input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "z", ctrlKey: true }));
      await Promise.resolve();
      expect(root.querySelector('[data-testid="editor-image"]')).toBeNull();
  ...
  it("drops an inline image at the resolved point", async () => {
    ...
    const instance = createOasisEditor(root);
    const input = root.querySelector('[data-testid="editor-input"]') as HTMLTextAreaElement;
    const editor = root.querySelector('[data-testid="editor-editor"]') as HTMLElement;
    ...
    try {
      ...
      const blocks = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-block"]'));
      ...
      try {
        ...
        for (let attempt = 0; attempt < 20; attempt += 1) {
          const image = secondBlock?.querySelector('[data-testid="editor-image"]');
          if (image) {
            break;
          }
          await new Promise((resolve) => setTimeout(resolve, 0));
      ...
      const blocksAfter = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-block"]'));
      expect(blocksAfter[0]?.querySelector('[data-testid="editor-image"]')).toBeNull();
      expect(blocksAfter[1]?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("extends the current selection with shift plus click", async () => {
    ...
    const applyRects = () => {
      const chars = root.querySelectorAll('[data-testid="editor-char"]') as NodeListOf<HTMLSpanElement>;
    ...
    const selectedChars = Array.from(root.querySelectorAll(".oasis-editor-char-selected")).map(
  ...
  it("toggles bold on the selected range with ctrl+b", async () => {
    ...
    const runNodes = root.querySelectorAll('[data-testid="editor-run"]');
  ...
  it("toggles bold from the toolbar", async () => {
    ...
    const runNodes = root.querySelectorAll('[data-testid="editor-run"]');
    expect((runNodes[runNodes.length - 1] as HTMLSpanElement).style.fontWeight).toBe("700");
    expect(boldButton.classList.contains("oasis-editor-tool-button-active")).toBe(true);
  ...
  it("applies a link from the toolbar prompt", async () => {
    ...
    const linkButton = root.querySelector('[data-testid="editor-toolbar-link"]') as HTMLButtonElement;
    linkButton.click();
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (document.querySelector('.oasis-editor-dialog-input')) break;
      await new Promise((resolve) => setTimeout(resolve, 0));
    }

    const linkInput = document.querySelector('.oasis-editor-dialog-input') as HTMLInputElement | null;
    expect(linkInput).not.toBeNull();
    if (linkInput) {
      linkInput.value = "https://example.com";
      linkInput.dispatchEvent(new Event("input", { bubbles: true }));
      const applyBtn = document.querySelector('.oasis-editor-dialog-button-primary') as HTMLButtonElement | null;
      applyBtn?.click();
    }

    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelector('[data-testid="editor-link"]')) break;
      await new Promise((resolve) => setTimeout(resolve, 0));
    }

    const link = root.querySelector('[data-testid="editor-link"]') as HTMLAnchorElement | null;
  ...
  it("edits the current link with ctrl+k when the caret is inside it", async () => {
    ...
    let linkInput = document.querySelector('.oasis-editor-dialog-input') as HTMLInputElement | null;
    if (linkInput) {
    ...
    input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "ArrowLeft" }));
    await Promise.resolve();
    expect(linkButton.classList.contains("oasis-editor-tool-button-active")).toBe(true);
    ...
    linkInput = document.querySelector('.oasis-editor-dialog-input') as HTMLInputElement | null;
    expect(linkInput).not.toBeNull();
    if (linkInput) {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (root.querySelector('[data-testid="editor-link"]')?.getAttribute("href") === "https://new.example") break;
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
  ...
  it("removes the current link with the unlink button", async () => {
    ...
    const unlinkButton = root.querySelector('[data-testid="editor-toolbar-unlink"]') as HTMLButtonElement;
    unlinkButton.click();
    await Promise.resolve();

    expect(root.querySelector('[data-testid="editor-link"]')).toBeNull();
  ...
  it("undoes and redoes link changes from the keyboard history flow", async () => {
    ...
    expect(root.querySelector('[data-testid="editor-link"]')).not.toBeNull();
  ...
  it("starts with style, font family and font size synced in the toolbar", () => {
    const root = document.getElementById("oasis-editor-root") as HTMLElement;
    const instance = createOasisEditor(root);
    ...
    expect(fontFamilySelect.value).toBe("Calibri, sans-serif");
  ...
  it("applies paragraph alignment and spacing from the toolbar", async () => {
    ...
    const alignCenterButton = root.querySelector(
      '[data-testid="editor-toolbar-align-center"]',
    ) as HTMLButtonElement;
    ...
    expect(alignCenterButton.classList.contains("oasis-editor-tool-button-active")).toBe(true);
  ...
  it("applies paragraph styles across all paragraphs touched by the selection", async () => {
    ...
    const paragraphs = Array.from(root.querySelectorAll('[data-testid="editor-block"]')) as HTMLParagraphElement[];
  ...
  it("toggles bullet and ordered lists from the toolbar", async () => {
    ...
    const bulletButton = root.querySelector(
      '[data-testid="editor-toolbar-list-bullet"]',
    ) as HTMLButtonElement;
    bulletButton.click();
    await Promise.resolve();
    ...
    const orderedButton = root.querySelector(
      '[data-testid="editor-toolbar-list-ordered"]',
    ) as HTMLButtonElement;
    orderedButton.click();
    await Promise.resolve();

    marker = root.querySelector('[data-testid="editor-list-marker"]') as HTMLSpanElement;
    expect(marker.textContent).toBe("1.");
    expect(orderedButton.classList.contains("oasis-editor-tool-button-active")).toBe(true);
  ...
  it("indents and outdents list items with tab and shift plus tab outside tables", async () => {
    ...
    let block = root.querySelector('[data-testid="editor-block"]') as HTMLElement;
    expect(block.style.marginLeft).toBe("0px");

    input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Tab" }));
    await Promise.resolve();

    block = root.querySelector('[data-testid="editor-block"]') as HTMLElement;
  ...
  it("numbers ordered lists by contiguous sequence at the same level", async () => {
    ...
    const markers = Array.from(root.querySelectorAll('[data-testid="editor-list-marker"]')).map(
  ...
  it("creates a new list item on enter and exits the list on empty enter", async () => {
    ...
    let markers = root.querySelectorAll('[data-testid="editor-list-marker"]');
    expect(markers.length).toBe(2);

    input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Enter" }));
    await Promise.resolve();

    markers = root.querySelectorAll('[data-testid="editor-list-marker"]');
  ...
  it("outdents and then removes the list with backspace at item start", async () => {
    ...
    let block = root.querySelector('[data-testid="editor-block"]') as HTMLElement;
    expect(block.style.marginLeft).toBe("0px");
    expect(root.querySelector('[data-testid="editor-list-marker"]')?.textContent).toBe("1.");

    input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Backspace" }));
    await Promise.resolve();

    expect(root.querySelector('[data-testid="editor-list-marker"]')).toBeNull();
  ...
  it("positions the caret after the list marker for an empty list paragraph", async () => {
    const root = document.getElementById("oasis-editor-root") as HTMLElement;
    const instance = createOasisEditor(root);
    const marker = root.querySelector('[data-testid="editor-list-marker"]') as HTMLElement | null;
  ...
  it("toggles page break before and keep with next from the toolbar", async () => {
    ...
    expect(pageBreakButton.classList.contains("oasis-editor-tool-button-active")).toBe(true);
    expect(keepWithNextButton.classList.contains("oasis-editor-tool-button-active")).toBe(true);
  ...
  it("imports a docx file into the editor state through the hidden file input", async () => {
    ...
    for (let attempt = 0; attempt < 10; attempt += 1) {
      if (root.querySelector('[data-testid="editor-block"]')?.textContent?.includes("Hello import")) {
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 0));
    }

    expect(root.querySelector('[data-testid="editor-block"]')?.textContent).toContain("Hello import");
  ...
  it("inserts an inline image through the hidden image input", async () => {
    ...
    try {
      ...
      imageInput.dispatchEvent(new Event("change", { bubbles: true }));
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const image = root.querySelector(".oasis-editor-image") as HTMLImageElement | null;
      ...
      input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "z", ctrlKey: true }));
      await Promise.resolve();
      expect(root.querySelector(".oasis-editor-image")).toBeNull();

      input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "y", ctrlKey: true }));
      await Promise.resolve();
      expect(root.querySelector(".oasis-editor-image")).not.toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("moves a selected inline image inside a table cell to the previous table cell with alt plus arrow up", async () => {
    ...
    try {
      ...
      const cellsBefore = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]'));
      const firstCellBefore = cellsBefore[0] ?? null;
      const secondCellBefore = cellsBefore[1] ?? null;
      const image = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement;
      expect(image).not.toBeNull();
      expect(image.closest('[data-testid="editor-table-cell"]')).toBe(secondCellBefore);
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const movedImage = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement | null;
        if (movedImage && movedImage.closest('[data-testid="editor-table-cell"]') === firstCellBefore) {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }

      const cellsAfter = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-table-cell"]'));
      expect(cellsAfter[0]?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
      expect(cellsAfter[1]?.querySelector('[data-testid="editor-image"]')).toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("moves a selected inline image before a table when moving up from the first paragraph", async () => {
    ...
    try {
      ...
      importInput.dispatchEvent(new Event("change", { bubbles: true }));
      for (let attempt = 0; attempt < 20; attempt += 1) {
        if (root.querySelector('[data-testid="editor-table"]')) {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const topLevelBlocks = Array.from(
          root.querySelectorAll<HTMLElement>('[data-testid="editor-surface"] > [data-block-id]'),
        );
        if (topLevelBlocks[0]?.querySelector('[data-testid="editor-image"]')) {
          break;
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }
      ...
      expect(topLevelBlocks[0]?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
      expect(topLevelBlocks[1]?.textContent?.replace(/\u00A0/g, "")).toBe("a");
      expect(root.querySelector('[data-testid="editor-table"]')).not.toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("supports undo and redo after dragging an inline image", async () => {
    ...
    try {
      ...
      const paragraphs = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-block"]'));
      ...
      try {
        ...
        for (let attempt = 0; attempt < 20; attempt += 1) {
          const movedImage = root.querySelector('[data-testid="editor-image"]') as HTMLImageElement | null;
          if (movedImage && movedImage.closest('[data-testid="editor-block"]') === targetParagraph) {
            break;
          }
          await new Promise((resolve) => setTimeout(resolve, 0));
        }
      ...
      const blocksAfterDrag = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-block"]'));
      ...
      expect(targetParagraphAfterDrag?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
      expect(originalParagraphAfterDrag?.querySelector('[data-testid="editor-image"]')).toBeNull();

      input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "z", ctrlKey: true }));
      await Promise.resolve();
      const blocksAfterUndo = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-block"]'));
      ...
      expect(targetParagraphAfterUndo?.querySelector('[data-testid="editor-image"]')).toBeNull();
      expect(originalParagraphAfterUndo?.querySelector('[data-testid="editor-image"]')).not.toBeNull();

      input.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "y", ctrlKey: true }));
      await Promise.resolve();
      const blocksAfterRedo = Array.from(root.querySelectorAll<HTMLElement>('[data-testid="editor-block"]'));
      ...
      expect(targetParagraphAfterRedo?.querySelector('[data-testid="editor-image"]')).not.toBeNull();
      expect(originalParagraphAfterRedo?.querySelector('[data-testid="editor-image"]')).toBeNull();
    } finally {
      Object.defineProperty(globalThis, "Image", {
        configurable: true,
        value: OriginalImage,
  ...
  it("round-trips a mixed document after editing an inline image", async () => {
    ...
    try {
      ...
      const fileDropdown = document.querySelector('[data-testid="editor-toolbar-file-dropdown"]') as HTMLElement;
      fileDropdown.click();
      await Promise.resolve();
