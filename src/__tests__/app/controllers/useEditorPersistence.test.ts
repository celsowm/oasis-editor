...
describe("useEditorPersistence", () => {
  ...
  it("should load document on mount", async () => {
    ...
    await createRoot(async (dispose) => {
      useEditorPersistence({ document: {} } as any, onLoaded, { enabled: true });
  ...
  it("should save document when it changes after initialization", async () => {
    ...
    await createRoot(async (dispose) => {
      useEditorPersistence(state as any, vi.fn(), { enabled: true });
  ...
  it("should not save document during initial load", async () => {
    ...
    await createRoot(async (dispose) => {
      useEditorPersistence({ document: { id: "initial" } } as any, vi.fn(), { enabled: true });
  ...
  it("should not load or save when disabled", async () => {
    ...
    await createRoot(async (dispose) => {
      useEditorPersistence(state as any, vi.fn(), { enabled: false });
