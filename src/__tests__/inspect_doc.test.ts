...
describe("inspect doc pagination", () => {
  const docxPath = process.env.OASIS_INSPECT_DOCX;
  const runIfDocxProvided = docxPath && fs.existsSync(docxPath) ? it : it.skip;

  runIfDocxProvided("paginates and logs content", async () => {
    ...
    layout.pages.forEach((page, pageIdx) => {
      console.log(`=========================================`);
      console.log(
        `PAGE ${pageIdx + 1} (Height: ${page.height}px / Max Height: ${page.maxHeight}px)`,
      );
      console.log(`=========================================`);

      page.blocks.forEach((block, blockIdx) => {
        console.log(
          `Block ${blockIdx + 1}: Type = ${block.blockType}, Height = ${block.estimatedHeight}px`,
        );
        ...
        if (
          ...
          if (block.tableSegment) {
            ...
            console.log(
              `  Table Segment: Rows ${startRowIndex} to ${endRowIndex} (starts: ${JSON.stringify(startRowCellBlockStarts)}, ends: ${JSON.stringify(endRowCellBlockEnds)})`,
            );
