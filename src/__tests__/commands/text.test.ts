...
describe("text commands", () => {
  ...
  describe("deleteBackward", () => {
    ...
    it("merges paragraphs when deleting at start of second paragraph", () => {
      const state = createEditorStateFromTexts(["one", "two"], {
        blockIndex: 1,
        offset: 0,
      });
      const nextState = deleteBackward(state);
    ...
    it.each<EditorUnderlineStyle>(["double", "dotted", "dash", "wave"])(
      ...
      (style) => {
        ...
        const applyUnderlineStyle = (
          current: EditorState,
          underlineStyle: EditorUnderlineStyle,
        ) => {
          const styleValue =
            underlineStyle === "single" ? null : underlineStyle;
          return toggleTextStyle(
            setTextStyleValue(current, "underlineStyle", styleValue),
            "underline",
          );
        };
  ...
  describe("moveOrCopySelectionToPosition", () => {
    ...
    it("moves multi-paragraph selection", () => {
      ...
      const next = moveOrCopySelectionToPosition(state, target);
      const texts = getParagraphs(next).map((p) =>
