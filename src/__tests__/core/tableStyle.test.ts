...
import { getParagraphs, paragraphOffsetToPosition, findParagraphTableLocation, type EditorState } from "../../core/model.js";
import {
...
describe("table style commands", () => {
  ...
  it("applies style only to selected column through the controller", () => {
    ...
    let state = createEditorStateFromTexts(["start"], { blockIndex: 0, offset: 5 });
    ...
    const ops = createEditorTableOperations({
    ...
    (ops as any).applySelectionAwareTextCommand((current: EditorState) => toggleTextStyle(current, "bold"));
