...
export async function patchRebuiltDocxPreservingSource(
  ...
  if (originallyPresent.styles) {
    ...
    rebuiltChanged =
      (await patchRebuiltStyleRunPropertiesFromSource(
        sourcePackage,
        rebuilt,
      )) || rebuiltChanged;
    rebuiltChanged =
      (await patchRebuiltTableStylePropertiesFromSource(
        sourcePackage,
        rebuilt,
      )) || rebuiltChanged;
  }
  ...
  rebuiltChanged =
    ...
    rebuiltChanged;
  rebuiltChanged =
    (await patchRebuiltNoteEntriesFromSource(
      document,
      sourcePackage,
      rebuilt,
    )) || rebuiltChanged;
  rebuiltChanged =
    (await patchRebuiltCommentEntriesFromSource(sourcePackage, rebuilt)) ||
    rebuiltChanged;
  rebuiltChanged =
