...
function isWrappedBoundary(
  previousRect: CharRect,
  nextRect: CharRect,
): boolean {
  return (
    nextRect.top > previousRect.top + 1 || nextRect.left < previousRect.left - 1
  );
}

function getCaretSlotCandidates(
  charRects: CharRect[],
  startOffset = 0,
): CaretSlotCandidate[] {
...
export function measureLinesFromRects(charRects: CharRect[]): MeasuredLine[] {
  ...
  for (let index = 1; index <= charRects.length; index += 1) {
    ...
    const wrapped =
      !atEnd && isWrappedBoundary(charRects[index - 1]!, charRects[index]!);
    ...
    const height = lineRects.reduce(
      (maxHeight, rect): number =>
        Math.max(maxHeight, normalizeHeight(rect.height)),
      0,
    );
    lines.push({
      index: lines.length,
...
export function getCaretSlotRects(charRects: CharRect[]): CaretSlotRect[] {
  return measureLinesFromRects(charRects).flatMap(
    (
      line,
      lineIndex,
      lines,
...
export function resolveClosestOffsetForBoundaryLine(
  ...
  const boundaryLine =
    boundary === "first" ? lines[0] : lines[lines.length - 1];
