...
export class TableFloatingToolbar {
    ...
    private createToolbar(): HTMLElement {
        ...
        const groups = [
            [
                { icon: "↑+", label: "Add Row Above", action: () => this.events.onAddRowAbove(this.currentTableId!) },
                { icon: "↓+", label: "Add Row Below", action: () => this.events.onAddRowBelow(this.currentTableId!) },
            ],
            [
                { icon: "←+", label: "Add Column Left", action: () => this.events.onAddColumnLeft(this.currentTableId!) },
                { icon: "+→", label: "Add Column Right", action: () => this.events.onAddColumnRight(this.currentTableId!) },
            ],
            [
                { icon: "Row-", label: "Delete Row", action: () => this.events.onDeleteRow(this.currentTableId!) },
                { icon: "Col-", label: "Delete Column", action: () => this.events.onDeleteColumn(this.currentTableId!) },
            ],
            [
                { icon: "🗑", label: "Delete Table", action: () => this.events.onDeleteTable(this.currentTableId!) },
            ]
        ];
