...
import {
  ...
  createTableRow,
  createTableCell,
} from "../document/DocumentFactory.js";
import {
...
function tryMergeSiblings(
  ...
  if (isTextBlock(prev) && isTextBlock(curr)) {
    ...
    (mergedBlock as any).children =
      mergedRuns.length > 0 ? mergedRuns : [curr.children[0]];
...
registerHandler(OperationType.APPLY_FORMAT, (state, op) => {
  ...
  if (
    ...
    let nextState: EditorState = { ...state, pendingMarks: { ...marks } };
  ...
  for (let i = firstBlockIdx; i <= lastBlockIdx; i++) {
    ...
    for (const r of targetBlock.children) {
      if (i === firstBlockIdx && r.id === startPos.inlineId)
        startAbs = acc + startPos.offset;
      if (i === lastBlockIdx && r.id === endPos.inlineId)
    ...
    nextState = updateDocumentSections(nextState, targetBlock.id, (b) => {
      ...
      if (merged.length === 0 && newChildren.length > 0)
        merged.push(newChildren[0]); // fallback
