...
export const paginateDocument = (documentModel, measure) => {
  ...
  for (const section of documentModel.sections) {
    const template = PAGE_TEMPLATES[section.pageTemplateId] ?? PAGE_TEMPLATES['template:a4:default'];
    ...
    for (const block of section.children) {
      ...
      if (currentY + composed.totalHeight > currentPage.contentRect.y + currentPage.contentRect.height) {
