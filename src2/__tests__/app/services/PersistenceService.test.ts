...
describe("PersistenceService", () => {
  ...
  it("should save and load a document", async () => {
    const doc = createEditor2Document([createEditor2Paragraph("Hello Persistence")]);
  ...
  it("should clear the saved document", async () => {
    const doc = createEditor2Document([createEditor2Paragraph("To be cleared")]);
    await service.saveDocument(doc);
  ...
  it("should handle updates to the same document", async () => {
    const doc1 = createEditor2Document([createEditor2Paragraph("Version 1")]);
    await service.saveDocument(doc1);

    const doc2 = { ...doc1, blocks: [createEditor2Paragraph("Version 2")] };
