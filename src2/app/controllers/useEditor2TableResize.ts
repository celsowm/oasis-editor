...
import type { Editor2State, Editor2TableNode } from "../../core/model.js";
...
export function createEditor2TableResize(deps: {
  state: () => Editor2State;
  ...
  const getTableAtEvent = (event: MouseEvent) => {
    ...
    if (target.closest(".oasis-editor-2-table-drag-handle")) {
        return null;
    }
    const table = target.closest(".oasis-editor-2-table-grid") as HTMLElement;
    const tableBlock = target.closest(".oasis-editor-2-table-block") as HTMLElement;
    if (!table || !tableBlock) return null;

    const tableId = tableBlock.getAttribute("data-source-block-id");
    if (!tableId) return null;

    const cells = Array.from(table.querySelectorAll(".oasis-editor-2-table-cell")) as HTMLElement[];
    const threshold = 12;
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    if (currentResizing) {
      if (currentResizing.type === "column") {
        document.body.classList.add("oasis-editor-2-hover-col-resize");
        document.body.classList.remove("oasis-editor-2-hover-row-resize");
      } else {
        document.body.classList.add("oasis-editor-2-hover-row-resize");
        document.body.classList.remove("oasis-editor-2-hover-col-resize");
      }
      return;
    }
  ...
  const handleMouseDown = (event: MouseEvent) => {
    ...
    const tableNode = allBlocks.find(b => b.id === info.tableId) as Editor2TableNode;
    ...
    if (Math.abs(delta) >= 2) {
      ...
      deps.applyTransactionalState((current) => {
        if (currentResizing.type === "row") {
          ...
          const tableNode = allBlocks.find(b => b.id === currentResizing.tableId) as Editor2TableNode;
          ...
          if (tableNode) {
              const layout = buildTableCellLayout(tableNode);
              const tableEl = document.querySelector(`[data-source-block-id="${currentResizing.tableId}"] .oasis-editor-2-table-grid`);
              if (tableEl) {
                  const cells = Array.from(tableEl.querySelectorAll(".oasis-editor-2-table-cell"));
