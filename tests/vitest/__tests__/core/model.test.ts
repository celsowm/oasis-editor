...
describe("resolveEffectiveParagraphStyle", () => {
  ...
  it("applies local paragraph overrides", () => {
    const result = resolveEffectiveParagraphStyle(
      { align: "center", spacingBefore: 12 },
      undefined,
    );
    expect(result.align).toBe("center");
    expect(result.spacingBefore).toBe(12);
    expect(result.spacingAfter).toBe(
      EFFECTIVE_PARAGRAPH_STYLE_DEFAULTS.spacingAfter,
    ); // default
  });
...
describe("resolveEffectiveTextStyleForParagraph", () => {
  it("inherits paragraph named style text attributes", () => {
    const result = resolveEffectiveTextStyleForParagraph(
      undefined,
      "normal",
      styles,
    );
...
describe("Page geometry helpers", () => {
  it("getPageContentWidth subtracts left + right margins", () => {
    ...
    expect(width).toBe(
      A4.width - A4.margins.left - A4.margins.right - A4.margins.gutter,
    );
  });
...
describe("getDocumentSections", () => {
  ...
  it("reads canonical sections", () => {
    ...
    const doc = {
      id: "doc:1",
      sections: [
        { id: "section:1", blocks: [sectionParagraph], pageSettings: A4 },
      ],
    };
...
describe("canonical block/paragraph helpers", () => {
  ...
  it("resolves active section blocks and zone blocks canonically", () => {
    ...
    expect(getActiveSectionBlocks(state as any).map((b) => b.id)).toEqual([
      "p:main",
    ]);
    expect(
      getEditableBlocksForZone(state as any, "header").map((b) => b.id),
    ).toEqual(["p:header"]);
    expect(
      getEditableBlocksForZone(state as any, "footer").map((b) => b.id),
    ).toEqual(["p:footer"]);
  });
});
